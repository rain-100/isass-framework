...
- `isass-web-springmvc` 已提供 `NocodeSpringMvcCrudRoute` 作为 v3 动态 controller 的默认 HTTP method/path descriptor，并通过 `NocodeSpringMvcCrudRequestFactory` 把 Spring MVC 解析出的 path/query/body 参数转换为框架无关的 `NocodeAccessRequest`，再由 `NocodeSpringMvcCrudEndpointInvoker` 交给 `NocodeAccessHandler` 执行；后续运行时注册端点时应复用这些边界，避免路由、文档和代码生成各自维护路径规则。
