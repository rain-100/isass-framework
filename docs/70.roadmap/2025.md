...
* [ ] 使用 vuepress-theme-vdoing 重写 isass 文档项目，支持多级目录（暂不实施）
* [ ] 文档项目只做文档框架，文档内容自动同步 isass 等具体项目的 md 文档，达到代码与文档同步的目的（暂不实施）

## 低代码模块
* [ ] （暂不实施）repository 从设计上，支持“有些数据库有，另一些数据库没有的 sql 语法”的业务查询兼容模式，以便能在支持特定 sql 语法的数据库上直接执行 sql，提高性能。首先实现 CTE 递归查询。
  - 2026-06-20：该项暂时取消，不进入当前 roadmap 实现队列，后续如重新启用再重新评估优先级和方案。
