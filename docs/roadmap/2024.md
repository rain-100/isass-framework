...
* [x]  模块重新命名，super 改为 isass，各模块的核心代码模块以 core 结尾
...
* [x]  使用java模块化技术重新划分模块，核心模块与 spring 解耦，便于非 spring 体系中使用。参考主流框架的方式，通过新建 spring-boot-starter/spring-boot-autoconfigure 模块，与 spring 集成。例如新建模块 isass-cache-spring-boot-starter
...
* [ ]  (暂停。放弃flyway。只用Liquibase替代)优化 flyway 单体与微服务适配的方式，不直接修改源码。
...
* [ ]  新增增删改一体的接口，方便前端一次性调用接口可以增删改数据，例如在多选框场景，用户可能新增勾选数据和取消勾选之前已经选择的数据
...
* [ ] 数据库版本管理工具由 flyway 迁移到 liquibase，以便方便支持多种数据库的版本管理
