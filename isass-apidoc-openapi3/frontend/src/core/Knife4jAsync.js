...
/**
 * 将仅由 $ref 组成的 oneOf 请求体转换为 Knife4j 现有的 body 参数模型。
 * 同时保留候选模型，供调试界面切换请求示例。
 */
SwaggerBootstrapUi.prototype.resolveOneOfBodySchema = function (schema, media, swpinfo) {
  var that = this;
  var oneOf = null;
  var array = false;
  ...
  if (!KUtils.arrNotEmpty(oneOf) || oneOf.some(item => KUtils.strBlank(item['$ref']))) {
    return false;
  }

  var mapping = KUtils.getValue(media, 'x-isass-oneof-mapping', {}, true);
  if (!KUtils.checkUndefined(mapping)) {
    mapping = KUtils.getValue(schema, 'x-isass-oneof-mapping', {}, true);
  }
  var options = [];
  oneOf.forEach(item => {
    var className = KUtils.getClassName(item['$ref'], swpinfo.oas2);
    var value = {};
    try {
      var definition = that.getDefinitionByName(className, swpinfo.oas2);
      if (definition != null && KUtils.checkUndefined(definition.value)) {
        value = definition.value;
      }
    } catch (error) {
      that.log('oneOf模型示例生成失败: ' + className);
    }
    var requestValue = array ? [value] : value;
    options.push({
      label: className,
      value: className,
      entityName: KUtils.getValue(mapping, className, '', true),
      requestValue: KUtils.json5stringify(requestValue, null, '\t')
    });
  });
  if (!KUtils.arrNotEmpty(options)) {
    return false;
  }

  swpinfo.oneOfOptions = options;
  swpinfo.oneOfSelected = options[0].value;
  swpinfo.oneOfArray = array;

  var selectedSchema = array
    ? { 'type': 'array', 'items': oneOf[0] }
    : oneOf[0];
  var parameter = that.bodyParameterResolverSchema(selectedSchema, swpinfo.oas2);
  if (!KUtils.checkUndefined(parameter)) {
    parameter = {
      'in': 'body',
  ...
  that.assembleParameterOAS3(parameter, swpinfo, []);
  swpinfo.requestValue = options[0].requestValue;
  return true;
}
...
SwaggerBootstrapUi.prototype.initApiInfoAsyncOAS3 = function (swpinfo) {
  ...
  var apiInfo = swpinfo.originalApiInfo;
  if (!swpinfo.init) {
    var criteriaParameters = KUtils.getValue(apiInfo, 'x-isass-criteria-parameters', null, true);
    if (KUtils.checkUndefined(criteriaParameters)) {
      swpinfo.criteriaParameterNames = criteriaParameters;
      swpinfo.criteriaOptions = Object.keys(criteriaParameters).map(entityName => ({
        label: entityName,
        value: entityName,
          ...
          for (var consume in bodyContent) {
            ...
            if (KUtils.checkUndefined(consumeBody) && consumeBody.hasOwnProperty('schema')) {
              ...
              if (swpinfo.enterprisePlugins.orangeforms) {
                ...
                if (that.resolveOneOfBodySchema(schema, consumeBody, swpinfo)) {
                  // oneOf 已转换为 body 参数
                }
                // 此处判断properties,如果有properties,说明有属性,非ref
                else if (schema.hasOwnProperty('properties') && KUtils.checkUndefined(schema['properties'])) {
