              ...
              <a-row v-if="urlFormFlag">
                <a-table v-if="urlFormTableFlag" bordered size="small" :rowSelection="rowUrlFormSelection"
                  ...
                  <template slot="urlFormValue" slot-scope="text, record">
                    ...
                    <a-row v-else>
                      <a-input :placeholder="record.description" :class="'knife4j-debug-param-require' + record.require"
                        :data-key="record.id" :value="text" @change="urlFormContentChange" />
                    </a-row>
                  </template>
                  <a-row slot="operation" slot-scope="text, record">
                    <a-button v-html="$t('debug.tableHeader.holderDel')" type="link" v-if="!record.new"
                ...
                <a-row v-if="rawFormFlag">
                  ...
                  <a-table v-if="rawFormTableFlag" bordered size="small" :rowSelection="rowRawFormSelection"
                    ...
                    <template slot="urlFormValue" slot-scope="text, record">
                      ...
                      <a-row v-else>
                        <a-input :placeholder="record.description" :class="'knife4j-debug-param-require' + record.require"
                          :data-key="record.id" :value="text" @change="rawFormContentChange" />
                      </a-row>
                    </template>
                    <a-row slot="operation" slot-scope="text, record">
                      <a-button v-html="$t('debug.tableHeader.holderDel')" type="link" v-if="!record.new"
...
export default {
  ...
  data() {
    return {
      ...
      oneOfOptions: [],
      oneOfSelected: null,
      criteriaAllFormData: null,
      criteriaAllUrlFormData: null,
      criteriaAllRawFormData: null,
      responseHeaders: [],
      responseRawText: "",
      responseCurlText: "",
      responseStatus: null,
  ...
  methods: {
    ...
    initOneOf(cacheApi) {
      this.oneOfOptions = this.api.oneOfOptions || [];
      if (this.oneOfOptions.length == 0) {
        return;
      }
      var cachedSelection = KUtils.checkUndefined(cacheApi)
        ? cacheApi.oneOfSelected
        : null;
      var selected = this.oneOfOptions.some(option => option.value == cachedSelection)
        ? cachedSelection
        : this.api.oneOfSelected;
      if (!this.oneOfOptions.some(option => option.value == selected)) {
        selected = this.oneOfOptions[0].value;
      }
      this.oneOfSelected = selected;
      this.applyOneOfSelection(selected, !KUtils.checkUndefined(cacheApi));
      var selectedOption = this.oneOfOptions.find(option => option.value == selected);
      this.filterCriteriaParameters(selectedOption ? selectedOption.entityName : selected);
    },
    oneOfChange(schemaName) {
      var selectedOption = this.oneOfOptions.find(option => option.value == schemaName);
      this.filterCriteriaParameters(selectedOption ? selectedOption.entityName : schemaName);
      this.applyOneOfSelection(schemaName, true);
    },
    applyOneOfSelection(schemaName, replaceRequestValue) {
      var option = this.oneOfOptions.find(item => item.value == schemaName);
      if (!KUtils.checkUndefined(option)) {
        return;
      }
      this.oneOfSelected = schemaName;
      this.api.oneOfSelected = schemaName;
      if (replaceRequestValue && KUtils.strNotBlank(option.requestValue)) {
      ...
      if (KUtils.strNotBlank(option.entityName)) {
        ...
        [this.formData, this.urlFormData, this.rawFormData].forEach(forms => {
          ...
          if (KUtils.checkUndefined(entityForm)) {
            entityForm.content = option.entityName;
            entityForm.value = option.entityName;
          }
        });
      }
    },
    filterCriteriaParameters(entityName) {
      var mapping = this.api.criteriaParameterNames || {};
      if (!KUtils.checkUndefined(mapping[entityName])) {
        return;
      }
      if (this.criteriaAllFormData == null) {
        this.criteriaAllFormData = this.formData.slice();
        this.criteriaAllUrlFormData = this.urlFormData.slice();
        this.criteriaAllRawFormData = this.rawFormData.slice();
      }
      var allNames = [];
      Object.keys(mapping).forEach(key => {
        allNames = allNames.concat(Object.keys(mapping[key] || {}));
      });
      var allowed = Object.keys(mapping[entityName] || {});
      var filter = item => !allNames.includes(item.name) || allowed.includes(item.name);
      this.formData = this.criteriaAllFormData.filter(filter);
    ...
    storeApiParams() {
      ...
      if (this.enableRequestCache) {
        ...
        cacheApi.oneOfSelected = this.oneOfSelected;
    ...
    setResponseCurl(resp) {
      ...
      var fullurl = "";
      var responseUrlIsComplete = false;
      if (httpReg.test(resp.responseURL)) {
        fullurl = resp.responseURL;
        responseUrlIsComplete = true;
      } else {
      ...
      if (this.rawFlag) {
        ...
        if (KUtils.strNotBlank(tmpUrlStr) && !responseUrlIsComplete) {
          // 地址栏追加参数
