 ...
 */
public final class V3FileStream {
    ...
    private final V3StreamBody body;
    ...
    public InputStream openInputStream() {
        ...
        try {
            ...
            consumption.set(Consumption.CLOSED);
            throw new IllegalStateException("无法创建 V3 文件读取流", exception);
        }
    }

    private void claim(Consumption next) {
        if (!consumption.compareAndSet(Consumption.NEW, next)) {
            throw new IllegalStateException("V3FileStream 已被消费；同一文件流只能选择 writeTo(OutputStream) 或 openInputStream() 之一");
        }
    }
    ...
    private static final class StreamInput extends FilterInputStream {
        ...
        private int checkRead(int count) throws IOException {
            ...
            throw new IOException("V3 文件流读取失败", exception);
        }
    }
}
