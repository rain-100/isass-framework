 ...
 * <pre>{@code
 * // 方式一：函数式（推荐）
 * List<Menu> tree = V2TreeEntityUtil.toTree(menus,
 *         Menu::getId, Menu::getParentId, Menu::getChildren, Menu::setChildren);
 *
 * // 方式二：反射
 * List<Menu> tree = V2TreeEntityUtil.toTree(menus, "id", "parentId", "children");
 *
 * // 方式三：hutool Tree builder
 * List<Menu> tree = V2TreeEntityUtil.toTree(menus, null,
 *         (menu, node) -> node.setId(menu.getId()).setParentId(menu.getParentId()),
 *         Menu::from,
 *         Menu::getChildren, Menu::setChildren);
 * }</pre>
 */
public final class V2TreeEntityUtil {

    private V2TreeEntityUtil() {
    }
