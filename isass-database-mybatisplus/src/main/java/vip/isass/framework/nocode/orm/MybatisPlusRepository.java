 ...
 */
@Slf4j
public abstract class MybatisPlusRepository<E extends IEntity<E>, C extends ICriteria<E, C>, M extends BaseMapper<E>>
    ...
    public int deleteCountByCriteria(ICriteria<E, C> criteria) {
        Wrapper<E> wrapper = WrapperUtil.getQueryWrapper(criteria);
        Assert.isTrue(!wrapper.isEmptyOfNormal(), "删除失败，删除条件不能为空");
    ...
    public int updateCountByCriteria(E entity, ICriteria<E, C> criteria) {
        ...
        Assert.isTrue(!wrapper.isEmptyOfNormal(), "更新失败，更新条件不能为空");
        if (criteria instanceof IUpdateCriteria<?> updateCriteria
                && updateCriteria.resolveNullValueMode() == NullValueMode.WRITE_NULL
        ) {
            if (entity.presentProperties().isEmpty()) {
                throw new IllegalArgumentException(
                        "WRITE_NULL 必须提供显式属性存在性；请使用实体 setter、传输绑定或 markPresentProperty");
            }
            ...
            for (TableFieldInfo field : tableInfo.getFieldList()) {
                if (entity.isPropertyPresent(field.getProperty())
                        && values.get(field.getProperty()) == null
